...
class OpenAIService {
  ...
  /// 일기 내용에서 감정 추출
  static Future<String?> extractEmotion(String content) async {
    try {
      print('감정 추출 요청: ${content.substring(0, content.length > 50 ? 50 : content.length)}...');
      
      final response = await http.post(
        Uri.parse('$baseUrl/diary/extract-emotion'),
        headers: {
          'Content-Type': 'application/json',
        },
        body: json.encode({
      ...
      print('감정 추출 응답 상태: ${response.statusCode}');
      print('감정 추출 응답 내용: ${response.body}');

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        return data['emotion'];
      } else {
        print('감정 추출 실패: ${response.statusCode}');
        print('응답 내용: ${response.body}');
        return null;
      }
    } catch (e) {
      print('감정 추출 중 오류 발생: $e');
      return null;
    }
  }
