...
class _DiaryEntryState extends State<DiaryEntry> with TickerProviderStateMixin {
  ...
  String _currentEmotion = 'neutral'; // 현재 일기의 emotion
  ...
  String getCategoryEmoji(Emotion emotion) {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // _currentPostId는 null로 유지 (새 일기 생성 시 필요)
    });
  ...
  Future<void> _loadDiaryData() async {
    ...
    try {
      ...
      if (diaryData != null) {
        ...
        print('==== [DEBUG] diaryData["emotion"]: ${diaryData['emotion']}');
        if (!mounted) return;
        setState(() {
          ...
          _currentEmotion = diaryData['emotion']?.toString() ?? 'neutral';
        });
  ...
  Future<String?> fetchTTS(String text) async {
    try {
      final appState = Provider.of<AppState>(context, listen: false);
      final audioUrl = await TTSService.textToSpeech(text, appState);
      return audioUrl;
    } catch (e) {
      print('TTS 요청 중 오류: $e');
      return null;
  ...
  Future<void> _playTTS() async {
    ...
    if (!appState.voiceEnabled) {
      // 음성 기능이 꺼져있으면 아무 동작도 하지 않음 (알림도 띄우지 않음)
      return;
    }
    ...
    if (url != null) {
      ...
      // 사용자 설정에 따른 볼륨 적용
      final volume = appState.voiceVolume / 100.0; // 0-100을 0.0-1.0으로 변환
  ...
  Future<void> _toggleMute() async {
    ...
    if (!_isTtsPlaying) {
      ...
      if (_isMuted) {
        // 사용자 설정에 따른 볼륨으로 복원
        final appState = Provider.of<AppState>(context, listen: false);
        final volume = appState.voiceVolume / 100.0;
        await _audioPlayer.setVolume(volume);
      } else {
  ...
  Future<void> _handleSave() async {
    ...
    try {
      // 일기 내용에서 감정 추출
      final extractedEmotion = await OpenAIService.extractEmotion(_entryController.text);
      print('추출된 감정: $extractedEmotion');
      
      // 감정 추출에 실패하면 기본값 사용
      final emotion = extractedEmotion != null 
          ? _parseEmotionFromString(extractedEmotion)
          : Emotion.neutral;
      
      print('최종 사용 감정: $emotion');
      
      // 추출된 감정과 사용자 선택 카테고리로 이모지 업데이트
      final appState = Provider.of<AppState>(context, listen: false);
      final selectedCategory = appState.selectedEmoticonCategory;
      final emojiUrl = getCategoryEmoji(emotion);
      
      // setState(() {
      //   _currentEmoji = emojiUrl;
      // });
      
      print('설정된 이모지 URL: $emojiUrl');
      ...
      if (_currentPostId != null) {
        ...
        if (success) {
          ...
          setState(() {
            ...
            _isSaved = true;
            _isEditMode = false;
            // 수정된 일기의 emotion 업데이트
            _currentEmotion = emotion.name;
        ...
        final newImages = _uploadedImages
            ...
            .whereType<String>()
            .toList();
        final postId = await DiaryService().createDiary(
          content: _entryController.text,
          emotion: emotion,
        ...
        setState(() {
          ...
          _isSaved = true;
          _isEditMode = false;
          // 새로 생성된 일기의 post_id와 emotion 저장
  ...
  /// 문자열을 Emotion enum으로 변환
  Emotion _parseEmotionFromString(String emotionStr) {
    final lowerEmotion = emotionStr.toLowerCase();
    switch (lowerEmotion) {
      case 'happy':
      case '행복':
      case '기쁨':
  ...
  Widget build(BuildContext context) {
    ...
    // 현재 일기의 emotion 값 추출 (diary_calendar.dart 방식과 동일하게)
    final emotion = _parseEmotionFromString(_currentEmotion);
    // diary_calendar.dart와 동일한 방식으로 이모지 URL 생성
    final emojiUrl = getCategoryEmoji(emotion);
                              ...
                              children: [
                                Row(
                                  children: [
                                    if (_isSaved)
                                      Container(
                                        width: 72, // 기존 48 -> 72로 크기 증가
                                        height: 72,
                                        decoration: BoxDecoration(
                                          color: AppColors.calendarBg, // 초록색 대신 배경색과 어울리게
                                          borderRadius: BorderRadius.circular(36),
                                        ),
                                        child: Center(
                                          child: emojiUrl.startsWith('http')
                                              ? Image.network(
                                                  emojiUrl,
                                                  width: 48, // 기존 24 -> 48로 크기 증가
                                                  height: 48,
                                                  fit: BoxFit.contain,
                                                  errorBuilder: (context, error, stackTrace) {
                                                    return const Text(
                                                      '😊',
                                                      style: TextStyle(fontSize: 48), // 크기 증가
                                                    );
                                                  },
                                                )
                                              : Text(
                                                  emojiUrl,
                                                  style: const TextStyle(fontSize: 48), // 크기 증가
                                                ),
                                        ),
                                      ),
                                    if (_isSaved)
