...
"""
강제 이모티콘 마이그레이션 스크립트
모든 사용자의 이모티콘 데이터를 Firebase URL로 업데이트
"""
...
async def force_migrate_emoticons():
    """모든 사용자의 이모티콘을 Firebase URL로 강제 업데이트"""
    try:
        # MongoDB 연결
        ...
        print("🔧 강제 이모티콘 마이그레이션 시작...")
        
        # 모든 사용자 설정 업데이트
        update_result = await user_settings_collection.update_many(
            {},  # 모든 문서
            {
                '$set': {
        ...
        print(f"✅ 총 {update_result.modified_count}명의 사용자 이모티콘 업데이트 완료")
        ...
        async for document in cursor:
            ...
            emoticons = document.get('emoticon_categories', {})
            
            print(f"사용자 {user_id}:")
            for category, urls in emoticons.items():
                if urls and isinstance(urls, list) and len(urls) > 0:
                    first_url = urls[0]
                    if first_url.startswith('http'):
                        print(f"  {category}: Firebase URL ✅")
                    else:
                        print(f"  {category}: {first_url} ❌")
                else:
                    print(f"  {category}: 빈 리스트 ❌")
        
        print(f"\n🎉 확인 완료! 총 {updated_count}명의 사용자 설정 확인됨")
...
if __name__ == "__main__":
    print("🚀 강제 이모티콘 마이그레이션 스크립트 실행")
    print("⚠️ 이 작업은 모든 사용자의 이모티콘을 Firebase URL로 변경합니다. 계속하시겠습니까? (y/N): ", end="")
    
    response = input().strip().lower()
    if response in ['y', 'yes']:
        asyncio.run(force_migrate_emoticons())
    else:
